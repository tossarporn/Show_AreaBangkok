...
if($res = mysqli_query($connection,$select_guest)){
    if(mysqli_num_rows($res) !=1){
        ...
        $update_guest = "UPDATE `guest_register` SET `guest_name`='{$guest_name}',`guest_lastname`='{$guest_lastname}',`guest_tel`='{$guest_tel}',
        `guest_num_house`='{$guest_num_house}',`guest_street`='{$guest_street}',`guest_distric`='{$guest_distric}',`guest_area`='{$guest_area}'  
        WHERE `id` ='{$guest_id}'";  
